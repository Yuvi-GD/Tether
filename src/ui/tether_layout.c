    ...
    if (my_flex && my_flex->fill_ratio == 0.0f && my_flex->explicit_size.y <= 0.0f) {
        ...
        if (h && tether_ecs_is_valid(h->parent)) {
            ...
            if (pnode && pnode->flow == TETHER_FLOW_ROW) {
                Tether_AlignY align = my_flex->override_align_y ? my_flex->align_self_y : pnode->content_align_y;
                if (align == TETHER_ALIGN_Y_FILL) {
        ...
        while (tether_ecs_is_valid(child)) {
            ...
            if (flex) {
                ...
                if (node->flow == TETHER_FLOW_ROW) {
                    ...
                    Tether_AlignY align = flex->override_align_y ? flex->align_self_y : node->content_align_y;
                    if (align == TETHER_ALIGN_Y_FILL) {
                        ch = inner_h - flex->margin.top - flex->margin.bottom;
                    } else if (align == TETHER_ALIGN_Y_CENTER) {
                        ch = item_h;
                    ...
                    if (flex->fill_ratio == 0.0f && flex->explicit_size.y <= 0.0f && align != TETHER_ALIGN_Y_FILL) {
                        ch = child_used_h;
                    }
                    ...
                    Tether_AlignX align = flex->override_align_x ? flex->align_self_x : node->content_align_x;
